...
Сниппеты (сырые данные с сайтов):
{chr(10).join(f"- {s}" for s in snippets)}

Твоя задача — выделить факты и оформить читаемую карточку ухода для русскоязычного пользователя.

Формат:
Название: <русское имя> ({latin_name})
Семейство / Тип: <садовое / комнатное и т.д.>
Свет: ...
Полив: ...
Температура: ...
Почва: ...
...
Используй только факты из сниппетов. Ничего не придумывай. 
Если чего-то нет — просто пропусти этот блок. Не пиши общих советов.
Ответ должен быть в читабельной форме, с переносами строк, без HTML и форматирован под Telegram.
"""
