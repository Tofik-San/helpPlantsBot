...
🎯 Цель: создать готовую карточку для Telegram-бота, пригодную для хранения и показа пользователю.

🔍 Алгоритм:
1. Проверь, что {latin_name} — корректное латинское название
2. Если подтверждено — сгенерируй JSON
3. Если данных по сорту мало — заполни только проверенное. Не придумывай.

🔒 Ограничения:
– Только JSON, без markdown, HTML, эмодзи, описаний, пояснений  
– Ровно 10 полей  
– Все значения — строки  
– Без вложенности  
– Макс. 300 символов на каждое поле  
– Без лишних ключей (image, id, notes и т.д.)

📦 Формат:
{{
  "name": "Русское название + (латинское)",
  "category_type": "...",
  "short_description": "...",
  "light": "...",
  "watering": "...",
...
⚠️ Антиошибки:
– Не используй англицизмы: "евоним" → ❌
– Поле `name`: всегда "Русское название (латинское)" — не выдумывай
– Поле `category_type`: обязательно с назначением и семейством
– Убери эмоциональные фразы, вроде "прекрасное украшение"
– Не генерализируй по роду. Только сорт. Нет данных — пропусти аккуратно
– В `name`: обязательно сорт в кавычках, латинское — строго в скобках. Пример: "Фикус 'Бенджамина' (Ficus benjamina)"
– В `short_description`: описывай внешний вид, форму листьев, размер. Не писать «тропическое», «декоративное» и т.п. как основное описание.
– В `watering`: нельзя писать «поддерживать постоянную влажность». Уточняй глубину подсыхания или частоту.
– В `fertilizer`: указывать тип удобрений (например, «для лиственных», «калийные»), периодичность и способ.
– В `insights`: всегда включать регион происхождения, особенности среды, уходовые лайфхаки. Не пиши общих предупреждений без пользы.
– Не используй англицизмы: "евоним" → ❌
+ Не используй англицизмы или транслитерации: "евоним" → ❌, "питоспорум" без уточнения — ❌

– В `watering`: нельзя писать «поддерживать постоянную влажность». Уточняй глубину подсыхания или частоту.
+ В `watering`: нельзя писать «поддерживать влажность». Только конкретные условия: глубина подсыхания, график.

– В `name`: обязательно сорт в кавычках, латинское — строго в скобках. Пример: "Фикус 'Бенджамина' (Ficus benjamina)"
+ В `name`: сорт в одинарных кавычках, латинское — строго в скобках. Если сорт не указан — ставь "—"
– В `name`: транслитерации (''евоним'') → запрещены. Используй русское ботаническое имя (например, ''Бересклет'')  
– В `watering`: запрещены фразы "лёгкое подсыхание", "допускается влажность", "не переливать". Используй конкретные интервалы или глубину (например, "2–3 см").
– В `name`: не транслитерируй сорта (''Зандау''). Используй оригинальное имя в кавычках: ''Xanadu''
– В `insights`: не дублируй условия из других полей. Добавляй контекст, лайфхаки или особенности среды
– В `name`: Используется только ботаническое русское название и сорт. Бытовые, торговые и разговорные наименования запрещены: ''резиновое дерево'', ''драконово дерево'', ''день-ночь'' → ❌
