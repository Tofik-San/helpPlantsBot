...
async def get_care_card_html(latin_name: str) -> str | None:
    ...
    try:
        ...
        if not data:
            prompt_text = fprompt_text = f"""
Ты — специалист по уходу за растениями. Твоя задача — составить карточку ухода, понятную для обычного человека. Используй только сниппеты ниже.
...
Сформируй карточку по шаблону:

Название: {russian_name}
Латинское: {latin_name}
...
📌 Правила:
– Каждый пункт начинай с новой строки и отделяй пустой строкой  
– Не пиши общих советов  
– Только факты из сниппетов  
– Ничего не выдумывай  
– Латинское имя не изменяй
"""
